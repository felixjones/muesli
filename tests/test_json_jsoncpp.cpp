...
using B = muesli::jsoncpp_backend;
namespace mu = muesli;
...
static Json::Value from_json(const std::string& s) {
    Json::CharReaderBuilder rb;
    Json::Value val;
    std::string errs;
    std::istringstream stream(s);
    Json::parseFromStream(rb, stream, &val, &errs);
    return val;
}
...
int main() {
    ...
    // -- 5: double -------------------------------------------------------
    {
        ...
        // JsonCpp may format as "3.25" or "3.2500..." -- verify round-trip
    ...
    // -- 15: map<string,int> -> JSON object ------------------------------
    {
        ...
        assert(j["a"].asInt() == 1 && j["b"].asInt() == 2);
