...
using B = muesli::rapidjson_backend;
namespace mu = muesli;
...
static std::string to_json(const muesli::rapidjson_value& v) {
...
static muesli::rapidjson_value from_json(const std::string& s) {
    muesli::rapidjson_value v;
    v.doc->Parse(s.c_str());
    v.val.CopyFrom(*v.doc, v.allocator());
    return v;
}
