...
static std::string to_json(const nlohmann::json& j) { return j.dump(); }
static nlohmann::json from_json(const std::string& s) { return nlohmann::json::parse(s); }
...
int main() {
    ...
    // -- 16: map<string,string> ------------------------------------------
    {
        ...
        auto j = fmt.serialize(val);
        assert(j.is_object());
        assert(j["greeting"] == "hello" && j["target"] == "world");
        auto d = fmt.deserialize(j);
        assert(d && *d == val);
    }
    ...
    // -- 20: variant<int,string> -----------------------------------------
    {
        ...
        assert(to_json(fmt.serialize(V{int32_t{7}})) == "{\"i\":0,\"v\":7}");
        assert(to_json(fmt.serialize(V{std::string("hi")})) == "{\"i\":1,\"v\":\"hi\"}");
        auto d1 = fmt.deserialize(from_json("{\"i\":0,\"v\":7}"));
    ...
    std::cout << "All nlohmann JSON tests passed!" << std::endl;
    return 0;
}
